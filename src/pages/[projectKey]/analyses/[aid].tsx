...
const DashBoard = () => {
  ...
  const {data: dashboardData, mutate: setDashboardData, loading} = useRequest(async () => {
    const res = await request.get<any, DashBoardInfo>(`/api/projects/${projectKey}/analyses/${aid}/tabs/${tab}?locale=zh-CN&apiTag=230A`)
  ...
  const {run: updateDashboardDataReq} = useDebounceFn(async (dbData: DashBoardInfo) => {
    ...
    const res = await request.put<any, DashBoardInfo>(`/api/projects/${projectKey}/analyses/${aid}/tabs/${tab}?locale=zh-CN&apiTag=230A`, dbData)
    res.content = JSON.parse(res.content as any)
